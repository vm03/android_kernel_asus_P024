...
{
    if(ekth3260_lcm_source == 1)
    {
    	printk("[elan] %s: Goodix IC, skip usb notify\n", __func__);
	return 0;
    }
...
{
    ekth3260_lcm_source = Read_second_source_LCM(); 

    printk(KERN_INFO "[elan] %s: ekth3260_lcm_source = %d\n", __func__, ekth3260_lcm_source);
    if(ekth3260_lcm_source == 0)
    {
	printk(KERN_INFO "[elan] %s: elan driver initialing...\n", __func__);
	return i2c_add_driver(&ektf_ts_driver);
    }
    else
    {
	printk(KERN_INFO "[elan] %s: skip elan driver initial\n", __func__);
	return 0;
    }
}
